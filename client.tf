#Resource
resource "vsphere_virtual_machine" "vm_client" {
  count            = var.prov_vm_num_client
   name            = "${var.prov_vmname_prefix_c}${format("%03d",count.index)}"
...
#portworx
  ...
  clone {
    ...
    customize {
        linux_options {
          host_name = "${var.prov_vmname_prefix_c}${format("%03d",count.index)}"
          domain    = var.pram_domain_name
        }        

        network_interface {
          ipv4_address = "${var.pram_ipv4_class}${count.index+var.pram_ipv4_host_client}"
  ...
  provisioner "file" {
      source      = "file/conf/nomad/nomad-client.hcl"
